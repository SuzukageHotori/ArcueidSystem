    ...
    public class RESTfulApiHandlerBase : IHttpHandler
    {
        ...
        private static Dictionary<String, String> GetParam(String sQueryString)
        {
            ...
            if (string.IsNullOrEmpty(sQueryString))
                return param;
            string[] ar = sQueryString.Split(new char[] { '&' },StringSplitOptions.RemoveEmptyEntries);

            foreach (var item in ar)
            {
                try
                {
                    var nFind = item.IndexOf('=');
                    if (nFind >= 0)
                    {
                        var sKey = item.Substring(0, nFind);
                        var sValue = item.Substring(nFind + 1, item.Length - nFind - 1);
                        param[sKey] = sValue;
                    }
                    else
                    {
                        var r = JsonToDictionary.ToDictionary(item).ToDictionary(k=>k.Key,v=>v.Value.ToString());
                        param =  param.Union(r).ToDictionary(k=>k.Key,v=>v.Value);
                    }
                }
                catch (Exception e)
                {
